...
<head>
	...
	<meta name="keywords" content="お問い合わせ内容確認,リクルート情報,プログラマ,SE,プログラミング,ソフト開発,ソフトハウス,アイプランニング,能力重視" />
	<meta name="description" content="お問い合わせ内容確認 | プログラム開発請負アイプランニング" />
	...
	<title>お問い合わせ内容確認 | プログラム開発請負 アイプランニング</title>
...
if (param.s != "内容を確認" && param.post!="0") {
	Response.Redirect("./error.html");
}

var text = "";

if (param.n == "" || param.t == "" || param.k == "" || param.m == "" || !check_mail_address(param.m) || param.c == "" ) {
	text += "入力に不備があります。\n";
}
...
%>
				...
				<h2 style="color:#0C0C0C;font-size:18px;font-weight:bold;">お問い合わせ内容に不備があります。</h2>
				...
				<br />
				<p>お名前(ふりがな)と連絡先(御社名,電話番号,メールアドレス)を入力し応募して下さい。<br />
					<b>※印は入力必須項目</b><br />
					入力いただいた連絡先に、担当よりご連絡を差し上げます。</p>
				<br />
				<form method="post" action="contact.asp" id="entry_form" name="entry_form" onsubmit="return validate(this);">
				<dl>
				  <dt>※氏名:</dt>
				  ...
				  <dt>御社名:</dt>
				  <dd><input type="text" size="40" name="corp" id="entry_corp" value="<% = Server.HTMLEncode(param.corp) %>" /></dd>
				  <dt>※電話番号:</dt>
				  ...
				  <dt>※問い合わせ内容:</dt>
				  <dd><textarea rows="20" cols="40" name="c" id="entry_comment"><% = Server.HTMLEncode(param.c) %></textarea></dd>
				</dl>
				<p><br /><span class="underline">受付が完了いたしますと入力いただいたメールアドレスに自動的に「受付完了」の<br />
					メールが届きます。@ipl.co.jpからのメールが受け取れるようにしておいて下さい。</span></p>
				<p><input type="hidden" name="post" value="0" /><br /></p>
				
				<p><input type="submit" name="s" value="内容を確認" />(必須項目(※)を入力しないと機能しません。)</p>
				</form>
				<br  />
				<p>送信された個人情報に関し、弊社、<a href="../privacy/index.html" target="_blank">個人情報保護方針</a>に則り、あなたのプライバシーを保護することに細心の注意を払い、問い合わせ以外の目的で使用することは御座いません。</p>
				</div>
<%
}else{
%>
				<h2 style="color:#0C0C0C;font-size:18px;font-weight:bold;">お問い合わせ内容の確認</h2>
				<div class="bodytext" style="font-size:14px;padding:12px;background-color:#F0F0F0;" align="justify">
				<br />
					入力された内容をご確認下さい。<br />
					内容に間違いがなければ、下記「この内容で送信する」ボタンより<br />
					送信を完了させて下さい。<br />
					<br />
					<br />
					...
					<tr><td align="center">御社名</td>
						<td style="padding:5px;background-color:#FFFFFF;">
						<% = Server.HTMLEncode(param.corp) %></td>
						</tr>
					...
					<tr><td align="center">問い合わせ内容</td>
						<td style="padding:5px;background-color:#FFFFFF;">
						<% = Server.HTMLEncode(param.c) %></td>
						</tr>
					</table>

					<form method="post" action="contactPost.asp" id="entry_form" name="entry_form" onsubmit="return validate(this);">
					  ...
					  <dd><input type="hidden" size="80" name="corp" id="entry_corp" value="<% = Server.HTMLEncode(param.corp) %>" /></dd>
					...
					<p><br /><span class="underline">受付が完了いたしますとあなたのメールアドレスに自動的に「受付完了」の<br />
						メールが届きます。@ipl.co.jpからのメールが受け取れるようにしておいて下さい。</span><br />
						入力いただいた連絡先に、担当よりご連絡を差し上げます。</p>
					<p><input type="hidden" name="post" value="1" /><br /></p>
					<p><input type="submit" name="s" value="この内容で送信する" /></p>
					</form>
				<p><br />送信された個人情報に関し、弊社、<a href="../privacy/index.html" target="_blank">個人情報保護方針</a>に則り、あなたのプライバシーを保護することに細心の注意を払い、問い合わせ以外の目的で使用することは御座いません。<br /><br /></p>
