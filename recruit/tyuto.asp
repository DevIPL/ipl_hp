...
<head>
	...
	<meta name="keywords" content="中途採用応募内容確認,リクルート情報,プログラマ,SE,プログラミング,ソフト開発,ソフトハウス,アイプランニング,能力重視" />
	<meta name="description" content="中途採用応募内容確認 | プログラム開発請負アイプランニング" />
	...
	<title>中途採用応募内容確認 | プログラム開発請負 アイプランニング</title>
...
if (param.ag == "" || param.bg == "" || param.cr == "" || param.n == "" || param.a == "" || param.t == "" || param.k == "" || param.m == "" || !check_mail_address(param.m)) {
	text += "入力に不備があります。\n";
}
...
%>
				...
				<h2 style="color:#0C0C0C;font-size:18px;font-weight:bold;">中途採用応募内容に不備があります。</h2>
				...
				<br />
				<p>あなたのお名前(ふりがな)と連絡先(住所,電話番号,メールアドレス)、職歴等を入力し応募して下さい。<br />
					<b>※印は入力必須項目</b><br />
					応募いただいた連絡先に、後日、採用担当よりご連絡を差し上げます。</p>
				<br />
				<form method="post" action="tyuto.asp" id="entry_form" name="entry_form" onsubmit="return validate(this);">
				<dl>
				  <dt>※氏名:</dt>
				  ...
				  <dt>※年齢:</dt>
				  <dd><input type="text" size="30" name="ag" id="entry_age" value="<% = Server.HTMLEncode(param.ag) %>" /></dd>
				  <dt>※最終学歴:</dt>
				  <dd><input type="text" size="30" name="bg" id="entry_background" value="<% = Server.HTMLEncode(param.bg) %>" /></dd>
				  <dt>※職歴：（経験の有無を含む）</dt>
				  <dd><textarea rows="5" cols="40" name="cr" id="entry_career"><% = Server.HTMLEncode(param.cr) %></textarea></dd>
...
%>
				<h2 style="color:#0C0C0C;font-size:18px;font-weight:bold;">中途採用応募内容の確認</h2>
				<div class="bodytext" style="font-size:14px;padding:12px;background-color:#F0F0F0;" align="justify">
				<br />
					入力された内容をご確認下さい。<br />
					...
					<br />
					<br />
					<table border="1" cellspacing="0" bgcolor="#F7F7F7" style="font-size:1.0em;color: #666666;" summary="応募内容">
					...
					<tr><td align="center">年齢</td>
						<td style="padding:5px;background-color:#FFFFFF;">
						<% = Server.HTMLEncode(param.ag) %></td>
						</tr>
					<tr><td align="center">最終学歴</td>
						<td style="padding:5px;background-color:#FFFFFF;">
						<% = Server.HTMLEncode(param.bg) %></td>
						</tr>
					<tr><td align="center">職歴</td>
						<td style="padding:5px;background-color:#FFFFFF;">
						<% = Server.HTMLEncode(param.cr) %></td>
						</tr>
					...
					<form method="post" action="tyutoPost.asp" id="entry_form" name="entry_form" onsubmit="return validate(this);">
					  ...
					  <dd><input type="hidden" size="30" name="ag" id="entry_age" value="<% = Server.HTMLEncode(param.ag) %>" /></dd>
					  <dd><input type="hidden" size="30" name="bg" id="entry_background" value="<% = Server.HTMLEncode(param.bg) %>" /></dd>
					  <dd><input type="hidden" size="30" name="cr" id="entry_career" value="<% = Server.HTMLEncode(param.cr) %>" /></dd>
					</dl>
					<p><br /><span class="underline">応募の受付が完了いたしますとあなたのメールアドレスに自動的に「受付完了」の<br />
